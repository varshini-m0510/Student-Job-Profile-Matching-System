...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `achievement_BEFORE_INSERT` BEFORE INSERT ON `achievement` FOR EACH ROW BEGIN
    IF EXISTS (
        SELECT 1 
        FROM achievement 
        WHERE SRN = NEW.SRN 
          AND Achievement_Name = NEW.Achievement_Name
    ) THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'Duplicate achievement for the same student is not allowed';
    END IF;
END */;;
DELIMITER ;
...
BEGIN
    ...
    GROUP BY s.SRN, s.Name, a.Degree, a.CGPA;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2024-11-13 12:04:38
